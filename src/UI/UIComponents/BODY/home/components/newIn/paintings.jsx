...
const useSetOfExampleCollectionLogic = (data) => {
  ...
  return { paintingsImagesSrc, paintingsImagesText, bioInfo };
};
...
const SetOfExampleCollectionSection = ({ data }) => {
  ...
  const { paintingsImagesSrc, paintingsImagesText, bioInfo } =
    useSetOfExampleCollectionLogic(data);

  const { textBioTitle, linkType, linkId } = bioInfo;
